...
from .tools import (
    ...
    capture_uploaded_images,
    register_reference_image,
)

DEFAULT_LLM_MODEL = os.environ.get("RENOVATION_LLM_MODEL", "gemini-3-flash-preview")
DEFAULT_SEARCH_MODEL = os.environ.get("RENOVATION_SEARCH_MODEL", DEFAULT_LLM_MODEL)
...
search_agent = LlmAgent(
    name="SearchAgent",
    model=DEFAULT_SEARCH_MODEL,  # google_search requires Gemini 2.0+ models
...
**FIRST STEP when images are present**: Call **capture_uploaded_images** to save uploads as artifacts,
then **register_reference_image** for each image as "current_room" or "inspiration" based on your analysis.
If the user uploaded one room photo, register it as current_room. If two images, register the existing
space as current_room and the style reference as inspiration.
...
Include registered artifact filenames in your summary (from register_reference_image / list_reference_images).
""",
    tools=[
...
**Before generating:** Call **list_reference_images** to get saved artifact filenames.
...
- current_room_photo: [filename from current_room_artifact if a room photo was uploaded; omit for text-only]
- inspiration_image: [filename from inspiration_artifact if inspiration was uploaded; omit otherwise]
