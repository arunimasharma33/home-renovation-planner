...
DEFAULT_RENOVATION_TEXT_MODEL = os.environ.get("RENOVATION_GEMINI_TEXT_MODEL", "gemini-3-pro-preview")
DEFAULT_RENOVATION_IMAGE_MODEL = os.environ.get("RENOVATION_GEMINI_IMAGE_MODEL", "gemini-3-pro-image-preview")
    ...
    try:
        ...
        **Output Format:** Create a single, flowing paragraph that reads like a professional photography brief. 
        ...
        rewritten_prompt_response = client.models.generate_content(
            model=DEFAULT_RENOVATION_TEXT_MODEL,
        ...
        model = DEFAULT_RENOVATION_IMAGE_MODEL
...
async def list_reference_images(tool_context: ToolContext) -> str:
    ...
    lines = [get_reference_images_info(tool_context)]
    current = tool_context.state.get("current_room_artifact")
    inspiration = tool_context.state.get("inspiration_artifact")
    if current:
        lines.append(f"Current room artifact: {current}")
    if inspiration:
        lines.append(f"Inspiration artifact: {inspiration}")
    return "\n".join(lines)


async def capture_uploaded_images(tool_context: ToolContext) -> str:
    """Capture images from the user's message and save them as reference artifacts."""
    if "reference_images" not in tool_context.state:
    ...
    if user_content and user_content.parts:
        ...
        for part in user_content.parts:
            ...
            base_name = f"reference_upload_{upload_index}"
            version = get_next_version_number(tool_context, base_name)
            filename = create_versioned_filename(base_name, version, ext)

            image_part = types.Part(inline_data=part.inline_data)
            await tool_context.save_artifact(filename=filename, artifact=image_part)

            tool_context.state["reference_images"][filename] = {
                "version": version,
                "type": "unlabeled",
                "index": upload_index,
            }
            ...
            logger.info(f"Captured uploaded image as artifact: {filename}")

    if captured:
        return (
            f"Captured {len(captured)} image(s): {', '.join(captured)}. "
            "Call register_reference_image for each to label as current_room or inspiration."
        )

    return "No new images found in the current message. If images were uploaded earlier, use list_reference_images."


async def register_reference_image(
    tool_context: ToolContext,
    artifact_filename: str,
    image_type: str,
) -> str:
    """Label a captured reference image as the current room or inspiration photo."""
    if image_type not in ("current_room", "inspiration"):
        return "image_type must be 'current_room' or 'inspiration'."

    if "reference_images" not in tool_context.state:
        tool_context.state["reference_images"] = {}

    if artifact_filename not in tool_context.state["reference_images"]:
        tool_context.state["reference_images"][artifact_filename] = {
            "version": 1,
            "type": image_type,
        }
    else:
        tool_context.state["reference_images"][artifact_filename]["type"] = image_type

    if image_type == "current_room":
        tool_context.state["current_room_artifact"] = artifact_filename
    else:
        tool_context.state["inspiration_artifact"] = artifact_filename

    tool_context.state["latest_reference_image"] = artifact_filename
    logger.info(f"Registered {artifact_filename} as {image_type}")
    return f"Registered {artifact_filename} as {image_type}."
